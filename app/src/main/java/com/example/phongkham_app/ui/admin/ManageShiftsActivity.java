...
public class ManageShiftsActivity extends AppCompatActivity {
    ...
    private void loadShiftData() {
        ...
        android.database.Cursor cursor = db.getAllDoctors();
        int i = 0;
        if (cursor != null && cursor.moveToFirst()) {
            int nameIndex = cursor.getColumnIndex("name");
            do {
                String doctorName = nameIndex != -1 ? cursor.getString(nameIndex) : "Bác sĩ";
                String shiftCount = String.valueOf((i % 3) + 3); // Gán mẫu ca
                addTableRow(doctorName, shiftCount, i % 2 == 0);
                i++;
    ...
    private void loadScheduleData() {
        ...
        com.example.phongkham_app.data.local.DatabaseHelper db = new com.example.phongkham_app.data.local.DatabaseHelper(this);
        android.database.Cursor cursor = db.getAllDoctors();
        java.util.List<String> doctorList = new java.util.ArrayList<>();
        if (cursor != null && cursor.moveToFirst()) {
            int nameIndex = cursor.getColumnIndex("name");
            do {
                if (nameIndex != -1) doctorList.add(cursor.getString(nameIndex));
            } while (cursor.moveToNext());
            cursor.close();
        }

        if (doctorList.isEmpty()) {
            doctorList.add("BS. Nguyễn Văn A");
            doctorList.add("BS. Trần Thị B");
        }
        ...
        for (int i = 0; i < 7; i++) {
            ...
            String morningDoctor = (i == 2 || i == 5) ? null : doctorList.get(i % doctorList.size());
            String afternoonDoctor = (i == 1 || i == 4) ? null : doctorList.get((i + 1) % doctorList.size());
