...
public class SettingsPatientActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (userId != -1) {
             com.example.phongkham_app.data.model.Patient p = com.example.phongkham_app.data.repository.PatientRepository.getInstance(this).getPatientProfile(userId);
             if (p != null) {
                 android.widget.TextView tvProfileName = findViewById(R.id.tvProfileName);
                 if (tvProfileName != null) tvProfileName.setText(p.getName());
             }
        }
    }
