    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnFinishPrescription.setOnClickListener(v -> {
            ...
            for (Prescription p : prescriptionList) {
                if (p != null && p.getDrugName() != null && !p.getDrugName().isEmpty() && !p.getDrugName().equals("Thuốc Mới")) {
                    sb.append(p.getDrugName())
                      .append(" (").append(p.getDosage() != null ? p.getDosage() : "X").append("), ")
                      .append(p.getFrequency() != null ? p.getFrequency() : "")
                      .append(" [ Số lượng: ").append(p.getQuantity() != null ? p.getQuantity() : "1").append(" ]")
                      .append(" | ");
                }
            }
            Intent resultIntent = new Intent();
            resultIntent.putExtra("prescriptions_text", sb.toString());
            setResult(RESULT_OK, resultIntent);

            Toast.makeText(this, "Đơn thuốc đã được đính kèm.", Toast.LENGTH_SHORT).show();
            finish();
        });
    }
