...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private void seedData(SQLiteDatabase db) {
        ...
        // 2. Doctors
        String[][] doctorsArgs = {
            {"Nguyễn", "Văn A", "doctor@phongkham.com", "Nha khoa thẩm mỹ", "10", "Chuyên gia về bọc răng sứ và tẩy trắng răng."},
            {"Trần", "Thị B", "doctor2@phongkham.com", "Chỉnh nha", "8", "Chuyên gia niềng răng Invisalign."},
            {"Lê", "Minh C", "doctor3@phongkham.com", "Tiểu phẫu", "12", "Chuyên phẫu thuật nhổ răng khôn."},
            {"Phạm", "Quốc D", "doctor4@phongkham.com", "Phục hình", "15", "Kinh nghiệm lâu năm trong làm răng giả."}
        };

        long[] docIds = new long[doctorsArgs.length];
        for (int i = 0; i < doctorsArgs.length; i++) {
            ContentValues doctorUser = new ContentValues();
            doctorUser.put("first_name", doctorsArgs[i][0]);
            doctorUser.put("last_name", doctorsArgs[i][1]);
            doctorUser.put("email", doctorsArgs[i][2]);
            doctorUser.put("password_hash", "doctor123");
            doctorUser.put("role", "DOCTOR");
            docIds[i] = db.insert(TABLE_USERS, null, doctorUser);
            
            ContentValues doctorProfile = new ContentValues();
            doctorProfile.put(COLUMN_ID, docIds[i]);
            doctorProfile.put("specialty", doctorsArgs[i][3]);
            doctorProfile.put("experience_years", Integer.parseInt(doctorsArgs[i][4]));
            doctorProfile.put("bio", doctorsArgs[i][5]);
            db.insert(TABLE_DOCTORS, null, doctorProfile);
        }
        long doctorId = docIds[0]; // For backward compatibility below
        ...
        insertServiceImages(db, s1, "Picture/khamvatuvan", new String[]{"khamvatuvan_anh1.png", "khamvatuvan_anh2.png", "khamvatuvan_anh3.png", "khamvatuvan_anh4.png"});

        long s2 = insertService(db, catGeneral, "Lấy cao răng", "Vệ sinh răng miệng", 200000, 30);
        insertServiceImages(db, s2, "Picture/laycaorangdanhbong", new String[]{"laycaorangdanhbong_anh1.png", "laycaorangdanhbong_anh2.png", "laycaorangdanhbong_anh3.png", "laycaorangdanhbong_anh4.png"});

        long s3 = insertService(db, catSurgery, "Nhổ răng khôn", "Tiểu phẫu răng khôn", 1500000, 60);
        insertServiceImages(db, s3, "Picture/nhorangkhon", new String[]{"nhorangkhon_anh1.png", "nhorangkhong_anh2.png", "nhorangkhon_anh3.png", "nhorangkhon_anh4.png"});

        long s4 = insertService(db, catSurgery, "Nhổ răng thường", "Nhổ răng bị hỏng không thể giữ lại", 500000, 30);
        insertServiceImages(db, s4, "Picture/nhorangthuong", new String[]{"nhorangthuong_anh1.png", "nhorangthuong_anh2.png", "nhorangthuong_anh3.png", "nhorangthuong_anh4.png"});

        long s5 = insertService(db, catGeneral, "Trám răng sâu", "Chữa và trám các răng bị sâu", 300000, 45);
        insertServiceImages(db, s5, "Picture/tramrangsau", new String[]{"tramrangsau_anh1.png", "tramrangsau_anh2.png", "tramrangsau_anh3.png", "tramrangsau_anh4.png"});

        long s6 = insertService(db, catGeneral, "Điều trị tủy răng", "Lấy tủy cho răng bị viêm", 1200000, 60);
        insertServiceImages(db, s6, "Picture/dieutrituyrang", new String[]{"dieutrituyrang_anh1.png", "dieutrituyrang_anh2.png", "dieutrituyrang_anh3.png", "dieutrituyrang_anh4.png"});

        long s7 = insertService(db, catAesthetic, "Bọc răng sứ", "Phục hình răng sứ thẩm mỹ", 3000000, 90);
        insertServiceImages(db, s7, "Picture/bocrangxu", new String[]{"bocrangxu_anh1.png", "bocrangxu_anh2.png", "bocrangxu_anh3.png", "bocrangxu_anh4.png"});

        long s8 = insertService(db, catOrthodontics, "Niềng răng", "Chỉnh nha, niềng răng thẩm mỹ", 30000000, 60);
        insertServiceImages(db, s8, "Picture/niengrang", new String[]{"niengrang_anh1.png", "niengrang_anh2.png", "niengrang_anh3.png", "niengrang_anh4.png"});

        long s9 = insertService(db, catAesthetic, "Tẩy trắng răng", "Tẩy trắng răng công nghệ cao", 1500000, 60);
        insertServiceImages(db, s9, "Picture/taytrangrang", new String[]{"taytrangrang_anh1.png", "taytrangrang_anh2.png", "taytrangrang_anh3.png", "taytrangrang_anh4.png"});

        long s10 = insertService(db, catDiagnosis, "Chụp X-quang", "Chụp phim X-quang răng", 200000, 15);
        insertServiceImages(db, s10, "Picture/chupxquang", new String[]{"chupxquang_anh1.png", "chupxquang_anh2.png", "chupxquang_anh3.png", "chupxquang_anh4.png"});
        ...
        for(int i=0; i<roomIds.length; i++) {
            ...
            appVal.put("doctor_id", docIds[i % docIds.length]);
        ...
        // Add explicit appointments for customer 1
        String todayString = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm", java.util.Locale.getDefault()).format(new java.util.Date());
        
        java.util.Calendar cal = java.util.Calendar.getInstance();
        cal.add(java.util.Calendar.DAY_OF_MONTH, -5);
        String pastString = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm", java.util.Locale.getDefault()).format(cal.getTime());
        
        cal = java.util.Calendar.getInstance();
        cal.add(java.util.Calendar.DAY_OF_MONTH, 3);
        String futureString = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm", java.util.Locale.getDefault()).format(cal.getTime());

        // 1. Past Appointment
        ContentValues pastAppt = new ContentValues();
        pastAppt.put("customer_id", customerIds[0]); 
        pastAppt.put("doctor_id", docIds[0]); 
        pastAppt.put("service_id", s1); 
        pastAppt.put("appointment_datetime", pastString);
        pastAppt.put("status", "COMPLETED");
        db.insert(TABLE_APPOINTMENTS, null, pastAppt);

        // 2. Today's Appointment
        ContentValues todayAppt = new ContentValues();
        todayAppt.put("customer_id", customerIds[0]); 
        todayAppt.put("doctor_id", docIds[1]); 
        todayAppt.put("service_id", s2); 
        todayAppt.put("appointment_datetime", todayString);
        todayAppt.put("status", "SCHEDULED");
        db.insert(TABLE_APPOINTMENTS, null, todayAppt);

        // 3. Future Appointment
        ContentValues futureAppt = new ContentValues();
        futureAppt.put("customer_id", customerIds[0]); 
        futureAppt.put("doctor_id", docIds[0]); 
        futureAppt.put("service_id", s3); 
        futureAppt.put("appointment_datetime", futureString);
        futureAppt.put("status", "PENDING");
        db.insert(TABLE_APPOINTMENTS, null, futureAppt);
    ...
    private void insertServiceImages(SQLiteDatabase db, long serviceId, String folder, String[] imageFiles) {
        for (int i = 0; i < imageFiles.length; i++) {
            ...
            values.put("image_url", folder + "/" + imageFiles[i]);
    ...
    public Cursor getAllServicesWithImages() {
        SQLiteDatabase db = this.getReadableDatabase();
        String query = "SELECT s.*, i.image_url " +
                       "FROM " + TABLE_SERVICES + " s " +
                       "LEFT JOIN " + TABLE_SERVICE_IMAGES + " i ON s.id = i.service_id AND i.is_thumbnail = 1 " +
                       "GROUP BY s.id " +
                       "ORDER BY s.name ASC";
    ...
    public Cursor getAllAppointmentsByCustomer(int customerId) {
        SQLiteDatabase db = this.getReadableDatabase();
