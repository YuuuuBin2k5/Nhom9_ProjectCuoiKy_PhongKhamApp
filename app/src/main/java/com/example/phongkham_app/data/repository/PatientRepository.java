...
public class PatientRepository {
    ...
    public Patient getPatientProfile(long userId) {
        Cursor cursor = dbHelper.getCustomerById((int) userId);
        if (cursor != null && cursor.moveToFirst()) {
            int fnIdx = cursor.getColumnIndex("first_name");
            int lnIdx = cursor.getColumnIndex("last_name");
            String firstName = fnIdx != -1 ? cursor.getString(fnIdx) : "";
            String lastName = lnIdx != -1 ? cursor.getString(lnIdx) : "";
            String name = (firstName + " " + lastName).trim();
            if (name.isEmpty()) name = "Bệnh nhân";
            cursor.close();
            // Mock other fields for now
            return new Patient(
                    "BN" + String.format("%08d", userId),
                    name,
                    "O+",
                    28,
                    "Nam",
